...
def svd_cardio_distribution_plot(path):

    data_set = 'cardio'
    x_train, y_train = load_data(path + 'data/' + data_set + '/train/')

    ica = TruncatedSVD(n_components=4)
...
def svd_loan_distribution_plot(path):
    ...
    plt.title('Financial Loan SVD Distribution')
    plt.savefig('plots/loan_svd_distribution.png')
